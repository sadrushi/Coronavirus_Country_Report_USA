...
plot4 <- ggplot(death_causes, aes(x = reorder(death_cause,+total), y = total, fill = death_cause)) + geom_bar(stat = "identity", show.legend = FALSE) + 
  labs(title = "Total number of Deaths for USA by Select Causes for years 2014 to 2021", subtitle = " Year: {closest_state}", x = "Cause of death", y = "Total count of deaths") +  geom_text(aes(label = label_value), position = position_dodge(0.9), vjust = -1) + transition_states(states = year) + enter_fade() + exit_shrink() + ease_aes('linear') + coord_flip()
